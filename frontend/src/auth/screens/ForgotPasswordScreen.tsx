...
export default function ForgotPasswordScreen() {
  const { forgotPassword, resetPassword } = useAppAuth();
  ...
  const onReset = async (e: React.FormEvent) => {
    ...
    try {
      await resetPassword(mobile, otp.trim(), password);
      navigate('/login');
    } catch (err) {
        ...
        <div className="rounded-[28px] p-px" style={{ background: 'linear-gradient(160deg, rgba(255,122,24,0.65), rgba(38,38,46,0.5) 40%, rgba(124,58,237,0.45))' }}>
          <div className="rounded-[28px] bg-[#0F0F14]/95 p-7 backdrop-blur" style={{ boxShadow: '0 24px 80px -24px rgba(255,122,24,0.35)' }}>
            ...
            <h1 className="mt-6 text-[28px] font-black leading-tight tracking-tight text-[#F5F5F7]">
              Reset your{' '}
              <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(90deg, #FF8F3D, #FF7A18)' }}>password</span>
            </h1>
            ...
            {!otpSent ? (
              <form onSubmit={onContinue} className="mt-6 space-y-4">
                ...
                <div className="flex items-start gap-2 rounded-xl border border-[#292B33] bg-[#15161C] px-3 py-2.5 text-xs font-semibold text-[#C99752]">
                  <Info size={15} className="mt-0.5 shrink-0" />
                  <span>Check your email — a verification code has been sent to your registered email address.</span>
                </div>
                ...
                <Field
                  ...
                  required
                  autoComplete="new-password"
                  right={<ToggleButton shown={showConfirm} onClick={() => setShowConfirm((v) => !v)} />}
                />
            ...
            <p className="mt-5 text-center text-sm text-[#A1A4AE]">
              Remembered it?{' '}
              <Link to="/login" className="font-bold text-[#FF8F3D] transition-colors hover:text-[#FF7A18]">Login</Link>
            </p>
          </div>
        </div>
      </div>
